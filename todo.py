...
class TodoListApp(tk.Tk):
    def __init__(self):
        ...
        self.task_input.insert(0, "type here")

        self.task_input.bind("<FocusIn>", self.clear_placeholder)
        self.task_input.bind("<FocusOut>", self.restore_placeholder)

        ttk.Button(self, text="Add",width=10, command=self.add_task).pack(pady=5)

        self.task_list = tk.Listbox(self, font=("TkDefaultFont", 16), height=10, selectmode=tk.NONE)
        self.task_list.pack(fill=tk.BOTH, expand=True, padx=10, pady=10)

        ttk.Button(self, text="Done", style="success.TButton",width=10,
                   command=self.mark_done).pack(side=tk.LEFT, padx=10, pady=10)
        ttk.Button(self, text="Delete", style="danger.TButton",width=10,
    ...
    def clear_placeholder(self, event):
        if self.task_input.get() == "type here":
            self.task_input.delete(0, tk.END)
